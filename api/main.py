...
from api.text_extract import extract as extract_from_text
...
def _pairwise_interactions(medications: list[str]) -> list[dict]:
    model = _load_model()
    if model is None:
        raise HTTPException(
            status_code=400,
    ...
    if missing:
        raise HTTPException(
            status_code=400,
            detail=f"PubChem could not resolve: {', '.join(missing)}",
        )
    ...
    for drug_a, drug_b in combinations(medications, 2):
        ...
        severity = (
            _labels[class_id]
            if 0 <= class_id < len(_labels)
            else DEFAULT_LABELS[position]
        )
        results.append(
            {
                "drug_a": drug_a,
                ...
                "tell_your_doctor": tell_your_doctor_sheet(drug_a, drug_b, severity),
            }
        )
...
def detect(request: MedicationRequest):
    ...
    if len(set(medications)) < 2:
        raise HTTPException(
            status_code=400,
            detail="Provide at least two distinct medication names",
        )
    results = _pairwise_interactions(medications)
...
def analyze(request: AnalyzeRequest):
    """Free-text intake: extract meds + herbs, then run interaction + herb checks."""
    extraction = extract_from_text(request.raw_text)
    medications = extraction["medications"]
    herbs = extraction["herbs"]
    unrecognized = list(extraction.get("unrecognized", []))

    interactions = []
    if len(medications) >= 2:
        interactions = _pairwise_interactions(medications)

    herb_flags = []
    for herb_name in herbs:
        record = HERBS_DB.get(herb_name)
        if record:
            herb_flags.extend(medication_flags(record, medications))

    response = {
        "matched_drugs": medications,
        "matched_herbs": herbs,
        "extraction_method": extraction["method"],
        "interactions": interactions,
        "herb_flags": herb_flags,
        "summary": (
            f"{len(interactions)} drug interaction(s), "
            f"{len(herb_flags)} herb flag(s) found"
        ),
        "unrecognized": unrecognized,
        "learned": bool(extraction.get("learned", False)),
        "new_learned": extraction.get("new_learned", []),
    }
    if unrecognized:
        if os.getenv("FEATHERLESS_API_KEY"):
            response["note"] = "unmatched items couldn't be matched"
        else:
            response["note"] = (
                f"unrecognized items: {unrecognized} — set FEATHERLESS_API_KEY "
...
def graph():
    if not GRAPH_PATH.exists():
        raise HTTPException(
            status_code=500,
            detail="Graph data not built — run build_graph.py",
        )
...
async def detect_herb(
    ...
    try:
        ...
        return {
            ...
            "method": method,
            "tell_your_doctor": (
                f"Oga doctor: dis one na {record['common_name']}, e fit interact "
                "with some of my medicines — abeg check am."
            ),
        }
    finally:
