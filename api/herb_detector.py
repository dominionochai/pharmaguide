...
def _identify_with_vision_api(image_path: str | Path, api_key: str | None) -> str | None:
    key = api_key or os.getenv("HERB_VISION_API_KEY") or os.getenv("FEATHERLESS_API_KEY")
    if not key:
        return None
    endpoint = os.getenv("HERB_VISION_API_URL", "https://api.featherless.ai/v1/chat/completions")
    try:
        ...
        payload = {
            "model": os.getenv("HERB_VISION_MODEL", "google/gemma-3-27b-it"),
            "messages": [
                {
                    "role": "user",
                    "content": [
