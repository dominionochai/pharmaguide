...
HERBS_DB = {
    ...
    "utazi": {"common_name": "utazi", "scientific_name": "Gongronema latifolium", "interaction_notes": "Traditionally used for blood sugar support; may add to the effect of antidiabetic medicines and insulin.", "severity": "moderate", "affected_drug_classes": ["antidiabetics", "insulin"]},
    "uziza": {"common_name": "uziza", "scientific_name": "Piper guineense", "interaction_notes": "Some Piper species affect platelet function; use caution alongside anticoagulant or antiplatelet medicines.", "severity": "minor", "affected_drug_classes": ["anticoagulants", "antiplatelets", "warfarin"]},
    "scent leaf": {"common_name": "scent leaf (nchanwu)", "scientific_name": "Ocimum gratissimum", "interaction_notes": "Studied for blood-sugar and blood-pressure lowering effects; may add to antidiabetic or antihypertensive medicines.", "severity": "moderate", "affected_drug_classes": ["antidiabetics", "antihypertensives"]},
    "alligator pepper": {"common_name": "alligator pepper", "scientific_name": "Aframomum melegueta", "interaction_notes": "Traditional use suggests mild effects on platelet function; use caution with anticoagulant or antiplatelet medicines.", "severity": "minor", "affected_drug_classes": ["anticoagulants", "antiplatelets", "warfarin", "aspirin"]},
...
ALIASES = {
    ...
    "gongronema latifolium": "utazi", "piper guineense": "uziza", "ocimum gratissimum": "scent leaf",
    "nchanwu": "scent leaf", "efirin": "scent leaf", "aframomum melegueta": "alligator pepper",
    "grains of paradise": "alligator pepper", "atare": "alligator pepper",
}
...
_KNOWN_DRUG_CLASSES = {
    "warfarin": {"warfarin", "anticoagulants", "antiplatelets"},
    "aspirin": {"aspirin", "antiplatelets", "anticoagulants"},
    "clopidogrel": {"antiplatelets", "anticoagulants"},
    "insulin": {"insulin", "antidiabetics"},
    "metformin": {"antidiabetics"},
    "glibenclamide": {"antidiabetics"},
    "lisinopril": {"antihypertensives"},
    "amlodipine": {"antihypertensives"},
    "digoxin": {"digoxin", "cardiac medicines"},
}
...
def medication_flags(record: dict, medications: list[str]) -> list[dict]:
    ...
    for medication in medications:
        ...
        if not matched:
            for token, token_classes in _KNOWN_DRUG_CLASSES.items():
                overlap = token_classes & class_set
                if token in med and overlap:
                    matched = sorted(overlap)
                    break
        if matched:
            flags.append({"medication": medication, "severity": record["severity"], "matched_classes": matched, "note": record["interaction_notes"]})
    return flags
